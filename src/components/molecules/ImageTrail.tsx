...
class ImageTrailVariant5 {
  ...
  private showNextImage() {
    ...
    const distance = Math.sqrt(dx * dx + dy * dy); // <-- change 'let' to 'const'
    if (distance !== 0) {
      dx /= distance;
      dy /= distance;
    }
...
export default function ImageTrail({ items = [], variant = 1 }: ImageTrailProps): JSX.Element {
  ...
  return (
    <div className="w-full h-full relative z-[100] bg-transparent overflow-visible" ref={containerRef}>
      {items.map((url, i) => (
        <div
          className="content__img w-[250px] aspect-[1.1] absolute top-0 left-0 opacity-0 overflow-visible [will-change:transform,filter]"
          key={i}
        >
          <div
            className="content__img-inner bg-center bg-contain bg-no-repeat w-[calc(100%+20px)] h-[calc(100%+20px)] absolute top-[-10px] left-[-10px]"
